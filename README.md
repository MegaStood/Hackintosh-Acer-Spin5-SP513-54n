...
| S3 sleep | 🧪 | **macOS-side driver issue, not a firmware limit (revised 2026-05-05).** Linux on the same hardware suspends and wakes cleanly with internal display intact — see [`docs/linux-i915-ddi-mapping-2026-05-05.md`](docs/linux-i915-ddi-mapping-2026-05-05.md). The Acer firmware DSDT exposes a working `_S3` package (`{0x05,0,0,0}`), `_PTS(3)` and `_WAK(3)` are reasonable, and the platform supports clean S3 sleep+wake end-to-end. Earlier "structurally broken on Ice Lake firmware" framing was wrong: the firmware is dual-mode (S3 + S0ix gated by Insyde NVS byte `S0ID`), and the user is in S3 mode. macOS S3 wake fails because `AppleIntelICLLPGraphicsFramebuffer` doesn't restore the eDP link on resume. Pending experiments: VBT-derived `framebuffer-conN-*` overrides (commit `5029ee0`) + `SSDT-EXT4-iGPU-Wake` post-`_WAK` Notify hook (commit `33c0fa6`) + retest. **Hibernate mode 25 still works as the validated fallback.** |
| External display (HDMI port) | ❌ | **No on-board LSPCON** (VBT child #2 confirmed `Onboard LSPCON: no`). The HDMI port socket is wired direct to DDI B's combo PHY but WhateverGreen on Ice Lake doesn't engage HDMI signaling cleanly; passive HDMI cable produces `sinkCount=0`/`portMode=1` (DP signaling on HDMI sink). Only viable path is an external active DP→HDMI adapter. See [`docs/linux-i915-ddi-mapping-2026-05-05.md`](docs/linux-i915-ddi-mapping-2026-05-05.md#2-hdmi-port-has-no-on-board-lspcon). |
| External display (USB-C DP alt-mode) | 🧪 | Hardware + firmware support is confirmed working on Linux (1920×1080 via Genesys hub on either USB-C port). macOS-side requires correct `framebuffer-conN-busid` overrides matching the real DDI letters (rear=DDI C/busid 2, front=DDI D/busid 3), now applied in commit `5029ee0`. Pending macOS-side validation. |
...
|---|---|---|---|---|
| Geekbench 5 single-core | 1,274 | ~1,300 (1287–1358 range) | **~−2 to −6%** | [Notebookcheck SP513-54N review](https://www.notebookcheck.net/Acer-Spin-5-SP513-Laptop-Review-Sensibly-Optimized-and-Balanced-13-Inch-Convertible.489330.0.html) (this unit, GB 5.5) vs [Geekbench Browser GB5 search for MacBookPro16,2](https://browser.geekbench.com/v5/cpu/search?q=MacBookPro16%2C2) (multiple runs) |
| Geekbench 5 multi-core | 3,979 | ~4,700 (4636–4798 range) | **~−14 to −17%** | (same sources) |
...
**The Geekbench 5 numbers are the most reliable comparison** because they're directly measured on this exact Spin 5 unit by Notebookcheck and benchmarked on the equivalent MBP16,2 across many runs in Geekbench's public browser. Single-core delta is ~2–6% (essentially indistinguishable for daily use, within run-to-run variance), multi-core delta is ~14–17% — explained by the SP513-54N's cooling envelope holding ~15W vs the MBP's vapor chamber holding ~28W under sustained load.

> **Note on Geekbench versioning:** GB 5.0 through 5.5 use the same scoring scale, so any GB5 result can be compared with any other GB5 result regardless of minor version. GB5 and GB6 are *not* directly comparable (different scaling). When linking to Geekbench Browser, use version-locked URLs like `browser.geekbench.com/v5/cpu/search?q=...` — the version-agnostic `browser.geekbench.com/macs/...` URLs default to whatever is the current major version (today: GB6) and will silently change scale over time.
